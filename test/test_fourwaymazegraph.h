...
using FourWayGraphFixture = AbstractMazeGraphFixture<FourWayStepMapGraph<true, uint16_t, uint16_t, ConcreteMaze::kWidth>, ConcreteMaze>;

template <typename ConcreteMaze>
class FourWayGraphTypedTest : public FourWayGraphFixture<ConcreteMaze>, public ::testing::Test {
};
using FourWayGraphTest = FourWayGraphTypedTest<Maze<16>>;

class FourWayGraphAffectedEdgesTest : public FourWayGraphFixture<Maze<16>>, public ::testing::TestWithParam<std::tuple<Position, unsigned int>> {
};
class FourWayGraphWallOnEdgeTest : public FourWayGraphFixture<Maze<16>>, public ::testing::TestWithParam<std::tuple<uint16_t, uint16_t, Position>> {
};
...
INSTANTIATE_TEST_SUITE_P(AffectedEdgesInvalid, FourWayGraphAffectedEdgesTest,
    ::testing::Values(
...
{
    Position p = mg.wallPositionOnEdge(std::get<0>(GetParam()), std::get<1>(GetParam()));
    EXPECT_EQ(std::get<2>(GetParam()), p);
}
INSTANTIATE_TEST_SUITE_P(WallOnEdgeGeneral, FourWayGraphWallOnEdgeTest,
    ::testing::Values(
        std::make_tuple(0, 16, Position { 0, 1 }),
        std::make_tuple(0, 1, Position { 1, 0 }),
        std::make_tuple(16, 0, Position { 0, 1 }),
        std::make_tuple(1, 0, Position { 1, 0 })));
INSTANTIATE_TEST_SUITE_P(WallOnEdgeInvalid, FourWayGraphWallOnEdgeTest,
    ::testing::Values(
        std::make_tuple(0, 32, kInvalidAgentState.pos),
